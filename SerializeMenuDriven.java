...
class FileClass
{
	// Writing into storage: Serialization...
	public static void writeToFile(ArrayList<Employee> list)
	{
		// Putting all the data back to the file 
		try
		{
			ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("EmployeeData\\Employee.ser"));
	...
	{
		// Loading all the data to the ArrayList
		try
		{
			File f1 = new File("EmployeeData\\Employee.ser");
			if(f1.exists())
			{
				ObjectInputStream ois = new ObjectInputStream(new FileInputStream("EmployeeData\\Employee.ser"));
...
public class SerializeMenuDriven
{
	...
	public void run()
	{
		...
		Input input = new Input();
		ArrayList<Employee> empObject = FileClass.readFromFile();
		...
		do{
			...
			switch(choice)
			{
				...
				case 2:
					...
					System.out.println("\tTotal number of employees joined this time " + Employee.empCount);
				...
				case 3:
					...
					Iterator<Employee> iteration1 = empObject.iterator();
					while(iteration1.hasNext())
						((Employee)iteration1.next()).raiseSalary();
				...
				case 4:
					System.out.println("\tTotal number of employees joined this time " + Employee.empCount);
					System.out.println("\tTotal Employee " + empObject.size());

					FileClass.writeToFile(empObject);
